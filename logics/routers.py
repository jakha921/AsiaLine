...
async def get_flights(db: Session = Depends(get_db), from_date: Optional[date] = datetime.now().date(), to_date: Optional[date] = datetime.now().date(), lang: Optional[str] = "ru"):
    ...
    # get id from sorted_flights and get for this competitors_prices and add to dict
    for flight in sorted_flights:
        print(flight.keys())
        flight["quotes"] = api.get_competitors_prices_by_flight_id(db, flight["id"])
